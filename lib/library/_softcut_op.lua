...
_softcut_op = function ( self, x, y, frame, grid)
  ...
  local playhead = util.clamp( self:listen(x + 1, y) or 1, 1, max_sc_ops )
  ...
  local l =  util.clamp( self:listen( x + 4, y ) or 0, 0, #self.chars ) -- level 1-z
  local r =  util.clamp( self:listen( x + 5, y ) or 0, 0, #self.chars ) -- rate  1-z
  local p =  util.clamp( self:listen( x + 6, y ) or 0, 0, #self.chars ) -- pos  1-z 
